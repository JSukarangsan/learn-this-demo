...
const ROUTES = [
  ...
  {
    // Raw notes count as raw wherever they're filed — the squad folder, or inside a
    // project when the meeting was about one project. Same shape as the design rule
    // above, and it has to stay ABOVE the projects catch-all or a project's meeting
    // notes get reported as a project update, which is the one thing they are not.
    test: (p) => p.startsWith('comms/notes/') || /^projects\/[^/]+\/comms\//.test(p),
    area: 'notes',
    significant: false,
    line: (n) =>
  ...
  },
  {
    ...
    area: 'comms',
    significant: false,
    line: (n) => `${n} ${plural(n, 'note')} filed — not decisions, just a record`,
  },
  {
