...
import { parseSources, check, sha256, byteLength, readFingerprint, readUpdated, overdueBy, findManifest } from './check.mjs'
...
describe('a source only a connector can reach', () => {
  ...
  test('fetch_via is reported as needing the skill, never skipped quietly', async () => {
    ...
    assert.equal(manual[0].priorUpdated, null, 'no recorded version yet, so a first pass')
  ...
  test('a connector-backed copy records a date, not a hash', async () => {
    // Tested against Glean on 2026-08-25: hashing what a connector returns is wrong,
    // because its payload carries parser metadata, every comment with timestamps, and a
    // percentRetrieved that changes with paging — all of which move without the document
    // moving. The modification time it reports is the source system's own and does not.
    writeFileSync(
      join(dir, 'team/_generated/h2-planning.md'),
      `<!-- Generated from https://docs.example/doc
     by /refresh-index on 2026-08-19.
...
`,
    ...
    assert.equal(manual[0].priorUpdated, '2023-10-24T01:04:20Z')
  })

  test('readUpdated accepts a bare date as well as a full timestamp', () => {
    const bare = join(dir, 'bare.md')
    writeFileSync(bare, 'Source updated: 2026-06-11\n')
    assert.equal(readUpdated(bare), '2026-06-11')
    const full = join(dir, 'full.md')
    writeFileSync(full, 'Source updated: 2026-06-11T19:44:13Z\n')
    assert.equal(readUpdated(full), '2026-06-11T19:44:13Z')
  })
})
